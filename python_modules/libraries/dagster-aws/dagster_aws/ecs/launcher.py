...
class EcsRunLauncher(RunLauncher, ConfigurableClass):
    ...
    def _check_use_current_task_definition_field(self, field_name: str, field_value: Any):
        if field_value != None:
            check.invariant(
                not self.use_current_task_definition,
                f"Cannot set {field_name} if use_current_task_definition is True",
            )
    ...
    def launch_run(self, context: LaunchRunContext) -> None:
        ...
        arn = tasks[0]["taskArn"]
        self._set_run_tags(run.run_id, cluster=task_config.cluster, task_arn=arn)
        self._set_ecs_tags(run.run_id, task_arn=arn)
        self.report_launch_events(run, arn, task_config.cluster)
    ...
    def _task_config(self, run, image, container_context) -> DagsterEcsTaskConfig:
        ...
        if self.use_current_task_definition:
            ...
            task_config = get_task_config_from_current_task(
                self.ec2, current_task_metadata.cluster, current_task, family
            )

        else:
            # Don't use the current task at all - purely derive the task to launch based on
            # the launcher config
            # TODO: use revisions as well here - tag a revision based on a hash of the task_config
    ...
    def _reuse_task_definition(
        ...
        # change dataclass to Namedtuple
